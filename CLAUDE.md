...
Service-layer regression coverage lives in `MoneyShot.Tests/` (xUnit). Run with `dotnet test MoneyShot.Tests/MoneyShot.Tests.csproj`. There is **no UI test coverage** — anything touching `EditorWindow`, `HistoryWindow`, or the tray menu requires manual verification on Windows; see the checklist in `DEVELOPER.md`.
...
`AutoUpdateService` polls `https://api.github.com/repos/Daolyap/MoneyShot/releases/latest`, downloads either the `.exe` or `.zip` asset, **verifies its SHA-256 against the `SHA256SUMS.txt` asset**, stages the new exe alongside the running one, then writes and launches a self-deleting batch script (`BuildWindowsSwapScript`) that waits for the current process to exit, swaps the binary, and relaunches. The owner/repo constants are hard-coded in `AutoUpdateService.cs:18-19`.

Version comparison is **SemVer-only** (Major.Minor.Patch via `CompareSemVer`) — the 4th `Version` part holds the CI build number which increments on every push. Comparing 4-part versions caused the "new version available" prompt on every relaunch even when the meaningful version was unchanged.
...
`EditorWindow` delegates undo to `MoneyShot/Editor/UndoController.cs`, which owns the `Stack<IUndoAction>` and a small set of action records (`AddElementUndoAction`, `RemoveElementUndoAction`, `CropUndoAction`, `ResizeUndoAction`). Action records call back into `EditorWindow` via `internal` hooks (`UndoAddElement`, `UndoRemoveElement`, `UndoCrop`, `UndoResize`) that mutate the canvas directly. Any new mutation that should be undoable needs to push a corresponding action — there is no automatic snapshotting. `ElementState` (in `MoneyShot/Editor/ElementState.cs`) is the lightweight record used by resize-undo to capture position + size + optional font size.

### Resize / drag — design notes

Cursor-mode mouse handling has caused multiple regressions. Rules that must be preserved:

1. **Each resize handle owns its own `MouseLeftButtonDown` handler** (`ResizeHandle_MouseLeftButtonDown` → `BeginResize`, or `EndpointHandle_MouseLeftButtonDown` → `BeginEndpointResize` for Line/Path). The handler marks the event `Handled = true` so `Canvas_MouseDown` never sees handle clicks. Don't go back to manual hit-testing inside `Canvas_MouseDown` — that path was a source of snap-to-(0,0) bugs because of NaN width fall-through.
...
3. **Use `MoneyShot.Editor.CanvasPosition.GetLeft/GetTop` for any read of `Canvas.GetLeft/GetTop`.** They handle the NaN case (Canvas attached properties default to NaN when never set, which silently propagates through arithmetic). Hand-rolling `if (double.IsNaN(left)) left = 0;` is forbidden — use the helper.
4. **Resize handles are visually 12×12 inside a 24×24 transparent hit-zone Rectangle.** This is for 4K usability. The visual is painted via a `DrawingBrush` so a single `Rectangle` carries both responsibilities — see `CreateResizeHandle` and `CreateEndpointHandle`.
5. **Line and Path (arrow) resize uses 2-handle endpoint mode**, not the 8-handle box. Endpoint resize state lives in `_isEndpointResizing`, `_originalEndpointStart`, `_originalEndpointEnd`. For `Path`, geometry is shifted in canvas-absolute coordinates (`ShiftPathGeometry`) so endpoint handles can be positioned without offset math.
...
- `MoneyShot.Services.Logger` is the logging facade. It writes to both `Debug.WriteLine` and a daily rolling file at `%AppData%\MoneyShot\logs\moneyshot-YYYYMMDD.log` (7-day retention). Don't add a heavier logging framework unless DI shows up for other reasons — see `Opus-Speaks.md` Section F.
